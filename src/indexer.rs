    ...
    let preparation = (|| -> Result<(_, _, _)> {
        ...
        for document in &inventory.documents {
            ...
            if !documentation_format_changed
                && let Some((id, old_hash, old_role)) = existing.get(&rel)
                && old_hash == hash
            {
                if old_role != role {
                    conn.execute("UPDATE files SET role=?1 WHERE id=?2", params![role, id])?;
            ...
            if let Some((old_id, _, _)) = existing.get(&document.file.path) {
                store::delete_file(conn, *old_id)?;
            }
            let identity = FileIdentity {
            ...
            let chunks = insert_markdown_file(conn, &identity, document)?;
            outcome.indexed += 1;
            outcome.chunks += chunks;
            published.insert(rel);
        }
...
/// Persist the Markdown chunk contract independently from the code extractor
/// contract. A format change reprocesses unchanged documentation without
/// needlessly invalidating unchanged code rows.
...
fn insert_markdown_file(
    ...
    let file = &captured.file;
        ...
        "Markdown file identity path does not match captured document"
    );
    ensure!(
        captured.bytes.len() as u64 == file.byte_len,
        "captured Markdown byte length does not match parser metadata for {}",
        file.path
    );
    ensure!(
        blake3::hash(&captured.bytes).to_hex().as_str() == file.content_hash,
        "captured Markdown hash does not match parser metadata for {}",
        file.path
    );
    ...
    for (position, chunk) in file.chunks.iter().enumerate() {
        ...
        let ordinal = i64::try_from(chunk.ordinal)
            .context("Markdown chunk ordinal exceeds SQLite integer range")?;
