...
fn tool_defs(profile: ToolProfile) -> Value {
    let mut tools = json!([
        ...
        {
            "name": "design_task",
            "description": "Run a bounded design-before-edit pass for one behavioral task. Localizes with hybrid search plus the structural graph, retains complete candidate declarations with explicit surrounding source windows, requires exact source evidence for mechanisms/detection/cure/touchpoints/oracle, and persists task-scoped design memory. Call this before editing when the task is causal, cross-file, or architecturally ambiguous.",
            "inputSchema": {
                "type": "object",
                "properties": {
...
mod tests {
    ...
    fn profile_instructions_explain_when_to_use_structural_traversal() {
        ...
        assert!(structural.contains("repository,workspace default"));
        assert!(structural.contains("repository alone excludes workspace-owned source"));
        ...
        assert!(structural.contains("retry must preserve the original task text exactly"));
    }
