...
pub fn scan(root: &Path, options: &CorpusOptions) -> Result<Corpus> {
    ...
    let fingerprint = corpus_fingerprint(&scanner.files);
    let files = std::mem::take(&mut scanner.files);
...
impl Scanner<'_> {
    fn walk_directory(&mut self, relative_directory: &Path) -> Result<()> {
        ...
        let entries = fs::read_dir(&directory)
            .with_context(|| format!("discover documentation under {}", directory.display()))?;
        let mut entries = entries
            .collect::<std::io::Result<Vec<_>>>()
            .with_context(|| format!("inventory documentation under {}", directory.display()))?;
