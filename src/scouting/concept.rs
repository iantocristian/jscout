...
mod tests {
    ...
    use super::{
        ConceptSource, ConceptSourceAlias, SourceSupport, Submission, normalize,
        submit_tool_schema, validate,
    };
    ...
    fn validated_claims_link_to_fingerprinted_children_without_copying_supports() -> Result<()> {
        ...
        let (input, relations) =
            super::annotate_input(&concept, &planned, "snapshot".into(), None)?;
    ...
    fn candidates_are_exhaustive_and_exactly_match_claim_citations() {
        ...
        let error =
            validate(&submission(excluded_cited), &planned).expect_err("cited source excluded");
        ...
        let error =
            validate(&submission(mismatched_link), &planned).expect_err("missing cited link");
