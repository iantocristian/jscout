...
| Semantic memory | Evidence-backed `workflow`/`annotation` artifacts, validated `annotate` write-back, defining/supporting workflow scope, freshness/degradation, search attachment, and memory inspection; top matching memory survives whole-response truncation when it fits |
...
| Broad semantic scouting | Bounded agent-authored workflow memory passed its fixed-snapshot budget replay, but free-form participant synthesis failed preflight coverage; broad generation now requires deterministic candidate enumeration plus exhaustive LLM classification |
