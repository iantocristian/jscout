...
| `src/docs/corpus.rs` | 2,548 | 27 inline (from `:1732`) | The documentation consumer of the shared traversal, plus the whole Markdown parser |
...
`src/docs/corpus.rs` and `src/docs/retrieval.rs` are the new entrants — together with `docs/store.rs` and `docs/mod.rs` they add 5,335 lines, and `src/commands/docs.rs` another 214. Two things about that mass are worth naming. First, `corpus.rs` is still four subsystems in one file: the documentation membership policy, the Markdown/MDX block parser, the chunk-budget splitter, and the glob validation the config loader calls at load time. What it no longer owns is the walk. `b92485c` and `03d5b50` moved the traversal engine into `src/walk/inventory.rs` (263 lines) behind a `RepositoryInventoryConsumer` trait (`src/walk.rs:58`), leaving `corpus.rs` 160 lines shorter and `src/walk.rs` with no module-level `use crate::docs` — the code-vs-docs descent policy is now readable in `walk/inventory.rs`, where a reader would look for it. The residue is a name collision: `corpus::repository_inventory` (`src/docs/corpus.rs:156`) and `walk::repository_inventory` (`src/walk.rs:187`) are different functions two hops apart in the same call chain, and only the module path distinguishes them at a call site. Second, the docs subsystem's tests are inline rather than in a sibling `tests.rs`, so the production/test ratio there (`corpus.rs` ≈ 1,731 production lines to 817 test lines) is visibly thinner than the convention elsewhere in the tree.
...
A `.md` edit does not start a watch generation. `EventClassifier::classify` routes source events through `walk::is_indexable`, whose `EXTENSIONS` list is JS/TS only (`src/walk.rs:10`), and the fall-through branch escalates only when `!path.is_file()` (`src/watch.rs:534`). The comment above it names "README edits" as ordinary repository noise.
...
Incremental scope saves nothing on the Markdown side anyway. `DocumentationCollector::finish` reads and fully parses every admitted document on every refresh (`src/docs/corpus.rs:337`, `:354`); the `unchanged` short-circuit at `src/indexer.rs:576` only skips the database write. Peak memory therefore holds the whole documentation corpus (up to 4 MiB per file) for the run, and that cap is a `CorpusOptions` field the indexer never overrides (`..CorpusOptions::default()`, `src/indexer.rs:388-390`, `DEFAULT_MAX_FILE_BYTES` at `src/docs/corpus.rs:16`) — so there is no `[docs]` key for it even though include/exclude are configurable.

The two planes also fail differently. A code read error becomes an `IndexRejection` surfaced by `report_rejections`; a document read error becomes a `doc_inventory` decision row (`src/docs/corpus.rs:410-415`) visible only through `jscout docs status`, so `outcome.rejected` undercounts documentation problems. Worse in the other direction: `insert_documentation_file` has no rejection path at all — every `ensure!` in it (`src/indexer.rs:915-932` and following) propagates out of the preparation closure and rolls back the entire index. One malformed captured document aborts the whole run. So does one stray FIFO or socket named `*.md` inside an active docs subtree: `inspect_special_file` is the one consumer callback that returns `Result`, and its `?` at `src/walk/inventory.rs:194` aborts the traversal through `ensure_regular_inventory_file` (`src/docs/corpus.rs:298`, `:459`). The same special file with any other extension is silently skipped.
...
`Command::root()` has a `Self::Config { command }` arm (`src/commands/mod.rs:86`) that `main` can never reach, since Config is dispatched first. Windows is handled in two places — `package-release.sh` picks `jscout.exe` for `*windows*` triples and `jscout.mjs:82` resolves `jscout.exe` on win32 — but no Windows target is built or declared, so `platformKey()` returns null first. `SearchHit.stub` (`src/docs/store.rs:56`) is computed and serialized and nothing consumes it. The `ensure!(defaults.enabled)` in the MCP documentation handler (`src/mcp.rs:1144-1147`) is unreachable in production and exists for a test-only entry point. `walk::source_inventory` still exists but is now diagnostics-only — `src/dependency.rs:731` and the workspace tests use it, and `walk::source_files` (`src/walk.rs:197`) wraps it for `src/commands/core.rs:489` and `:522`. Ignore semantics are therefore implemented twice inside one module, and the two walkers sort code paths with different keys (`files.sort()` on `Path` at `src/walk.rs:181` versus `as_os_str()` byte order at `src/walk/inventory.rs:207`), so the parity assertions pass on fixture luck rather than structure.

One test fixture actively misinforms: `src/store.rs:2099-2103` creates `vec_doc_embeddings_2` with a `snapshot TEXT PARTITION KEY` column that production's `ensure_vector_table` (`src/docs/retrieval.rs:973-983`) never creates. Reading the vec0 doc-table shape off that test gives the wrong answer.

## Testing and release gaps

570 Rust tests all run under one `cargo test`. The entire 5,549-line docs subsystem is gated by nothing else: neither CI smoke test runs `jscout docs`, and the release-package smoke (`.github/workflows/ci.yml:85-96`) exercises only `llm doctor` and `checker doctor`. Outside Rust, 71 test cases run in no workflow — 60 across 18 `scripts/*.test.mjs`, 4 in `examples/graph-memory/demo.test.mjs`, 7 in `inference/test_service.py` — and the root `package.json` test script hand-enumerates paths rather than globbing, omitting one file outright.
...
- **Decision rows cover only documentation candidates.** They do not: the walk's symlink branch calls `record_decision` for any symlink, whatever its extension, whenever the consumer is active on that subtree, and it does so without consulting `path_relevant` (`src/walk/inventory.rs:162-167`).
