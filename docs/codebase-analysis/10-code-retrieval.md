...
flowchart TD
  ...
  GATE0 --> SNAP["with_read_snapshot + current_snapshot"]
  GATE1 --> SNAP
  ...
  BUD --> OUT["compact JSON or pretty diagnostic JSON"]
```

`SNAP` is `store::with_read_snapshot(conn, "jscout_search", ...)`, a savepoint that rolls back on error; the snapshot id from `structural::current_snapshot` is stamped into the envelope and into every cursor. Both branches reach `BUD` — but only the exhaustive branch can fail there with a typed error, and only the exhaustive branch has metadata that must be recomputed after each shed.
...
| Cursor prefix | `jscout-exhaustive-v2` | `src/search.rs:1044` |
...
**Scope canonicalization.** `exhaustive_scope` (`src/search.rs:1159`) reorders roles and origins into the constant order of `file_role::ALL` / `origin::ALL` and collapses a full role list to the string `"all"`. `exhaustive_request_fingerprint` (`src/search.rs:1184`) blake3-hashes a domain tag, the raw query, and those normalized lists. The `scope` object echoed to the caller (`corpus: "indexed_chunks"`, `file_roles`, `origins`, `snapshot`) states what completeness was claimed over — echoing raw request arrays would misstate it, since an empty role filter means *every* role.
...
**Cursors.** `ExhaustiveCursorPosition` (`src/search.rs:1047`) is `{path, start, hash}` — a chunk's logical identity, not its row id, because row ids are disposable across index passes. `encode_exhaustive_cursor` (`src/search.rs:1232`) emits `jscout-exhaustive-v2 . snapshot(64 hex) . fingerprint(64 hex) . path(2 hex per byte) . start(16 hex) . hash(64 hex)`; the path is hex-encoded byte-by-byte because a path containing a dot would desynchronize field parsing. `decode_exhaustive_cursor` (`src/search.rs:1245`) validates prefix, field count, and every field width before comparing snapshot (distinct error message) and fingerprint (one message covering any query or scope change). `exhaustive_cursor_position` (`src/search.rs:1304`) then re-resolves the triple to a row id, requiring `COUNT(*) == 1` — it bails identically on zero (out of scope) and on more than one, so two chunks in one file with identical start offset and content hash make a cursor permanently unusable.
...
**Hits.** Each hit carries `score: 0.0`, `MatchReason::Lexical`, empty snippet/uses/used_by, projected anchors, and `match_lines`. The complete copy-safe follow-up argument object is granted at most once per traversal, on the first page only (`cursor.is_none()`, `src/search.rs:1498-1501`), and only to a hit with `anchors.len() <= 1` — an ambiguous hit never gets copy-pasteable arguments.
...
3. one hit's follow-up arguments (compact transport only)
4. `exhaustive_locator_only`, set once (compact + exhaustive)
5. a hit from the tail, never the last one
6. a `used_by` entry, then a `uses` entry
7. an anchor — guarded by `result.exhaustive.is_none()` (`src/search.rs:2613-2621`), so an exhaustive locator is never shortened
8. truncate the largest snippet by `max(overshoot, 128)`
9. give up
...
Measurement is a nested fixed point, because `rendered_bytes`, `byte_limit`, and `transport_sections` are themselves serialized fields of the thing being measured. `settle_rendered_bytes` (`src/search.rs:2721`, ≤8 iterations) sits inside `settle_search_response` (`src/search.rs:2735`, another ≤8 for section accounting, skipped when compact) inside `capture_unbudgeted_bytes` (`src/search.rs:2752`, a third ≤8, always run). Compact therefore has two nested fixed points; diagnostic has three. Each iteration is a full serialization.
...
The boundary is enforced in three places rather than one, and their strengths differ. Only `exact_definition_chunks` reads through the `code_chunks`/`code_files` views; code vector search joins `code_chunks` (`src/embed.rs:1804-1806`). Everything else in this file — `bm25_ranking`, `exhaustive_hits`, `exact_occurrence_chunks`, `prefilter_ranking_by_role`, `load_hit` — still joins bare `chunks`/`files`, and their docs exclusion rests entirely on the insert-side split keeping docs rows out of `chunks_fts` and out of `refs`/`member_calls`/`entity_sites`. Any future writer that mirrors docs into `chunks_fts` would open the code path silently. Two tests pin the current state: `markdown_admission_does_not_change_serialized_code_search_ranking` (`src/search/tests.rs:31`) asserts the serialized ranked response is byte-identical (modulo snapshot id) after adding a README repeating the query terms, and `docs_corpus_named_chunks_cannot_enter_code_exact_lookup` (`src/search/tests.rs:67`) hand-inserts a docs-corpus file with a *named* chunk and asserts both search and `query::find_symbols_in_origins` return nothing. A third check in `src/docs/retrieval.rs:2249-2256` asserts `embedding_index_entries ⋈ doc_chunk_meta = 0`.
...
- `exhaustive_locator_only` is guarded by `compact`, and diagnostic JSON has no follow-ups projection, so the same byte limit yields a different floor on the two transports.
...
31 `#[test]` functions in `src/search/tests.rs` (2,178 lines) plus 16 in `src/compact/tests.rs` (815 lines). Exhaustive coverage is unusually complete: `exhaustive_search_pages_the_complete_lexical_chunk_set_and_binds_its_cursor` (`:456`) pages a three-file corpus at limit 2 and asserts no duplicated or missing chunk, stable path order, distinct cursors, `truncated == next_cursor.is_some()`, and that duplicate or unsorted origins canonicalize to `[repository, workspace]`. `exhaustive_budget_locator_floor_advances_from_the_last_rendered_hit` (`:807`) drives the typed error, retries at exactly `minimum_bytes`, checks the locator carries `at` plus an anchor but no `followups`, and verifies the continuation returns exactly the expected remainder. Exact-tier ordering is tested against a hostile fake reranker (`:277`). Compact tests fix size targets rather than exact strings — eight ordinary hits under 4,000 bytes (`src/compact/tests.rs:298`), ten checker edges under 8,000 (`:526`). There is no fuzzing; `contains_code_identifier` is exercised through targeted cases only, and the reranker is driven by injected closures, never over HTTP.
