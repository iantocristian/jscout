...
| `who_uses` | both | `symbol` XOR `anchor` | `snapshot` (anchor mode only), `origins`, `formats`, `response_bytes`, `debug` | usage sites by confidence; `resolution` appears only when an exact anchor resolves differently | `query` + `compact::who_uses_string` |
