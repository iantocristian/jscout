...
`src/agent.rs` embeds the two tiered skills, `integrations/jscout/core/SKILL.md` and `integrations/jscout/full/SKILL.md`, as `CORE_GUIDE` and `FULL_GUIDE` (G28). `agent-guide` prints the selected `--tier` (default `core`) to stdout; `agent-guide --install ROOT --dest agents|claude|codex` canonicalizes the root, creates the destination's `skills/jscout/` directory, and opens the target with `create_new(true)`, so an existing project-local guide is an error, never an overwrite. `agent-guide --update` replaces one exact destination through a staged sibling and a single rename, and requires explicit `--tier` and `--dest` (`resolve_selectors`) so an update can neither target another destination by default nor downgrade an installed tier.

Both tiers teach **Flow 1** (investigate a known identifier): `semantic_search` at `exhaustive: true`, `limit` as a page size never a completeness boundary, `next_cursor` copied unchanged until `truncated` is false *and* the summed page-local `returned` equals `total_chunks`, deterministic budget recovery, then `definition` with a returned `sym:` anchor plus the response `snapshot`. A **scope transfer** rule keeps the original query and any explicit `origins`/`formats`/`file_roles` fixed while paging (the cursor fingerprints them) and carries explicitly supplied `origins`/`formats` into `definition` and `who_uses` without synthesizing them from the echoed `scope`. **Flow 2** localizes a fuzzy description with one ranked search. The full tier adds **Flow 3** (inquiry through `semantic_memory`, one expansion, `neighborhood` for exact-anchor drill-down) and the `annotate` write-back rules. Both tiers route documentation questions to `documentation_search` as a separate layer whose hits are authored prose, not runtime proof; responses no longer emit per-hit `followups` argument objects.

The unit tests in `src/agent.rs` pin 25 exact substrings shared by both tiers, assert the core tier stays under the G28 byte gate and never names a full-profile tool, and pin the frontmatter description's trigger form ("Use the jscout repository index to search…"). The guide is prose compiled into a binary, and a substring test is the cheap way to stop an edit from dropping a load-bearing rule; the cost is that rewording any pinned phrase breaks the build, so the test both protects and freezes the wording.
...
One behavioral test exists: `src/commands/core_tests.rs` (51 lines, `#[path]`-included at `src/commands/core.rs:485`) indexes a temp repository with two same-named `run` methods and asserts `cli_who_uses_for_target` gives each target exactly one `likely` usage and one `possible` — that resolved calls do not leak into another target's candidate set. `src/agent.rs` tests pin the per-tier contract markers, the core byte gate, install/update per destination, and the explicit-selector rule for `--update`.
