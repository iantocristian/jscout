...
`docs status` is the only leaf that runs on a disabled corpus. `src/commands/docs.rs:18-28` short-circuits before opening anything and prints `{"enabled": false, "active_corpus": false}` or a single human line. Otherwise it opens read-only and renders `store::status` — snapshot id, canonical root, file/chunk/embeddable/rejection/cached-embedding/ready-generation counts, then one line per admission decision (rule, subject, path, with base64 path when the path is not clean UTF-8) and one line per file whose front matter parsed as `malformed_as_body`.

`docs embed` is the only path in the whole binary that materializes documentation vectors — `jscout index` and `jscout embed` never do, which keeps documentation admission from changing code-embedding cost or timing. It requires the shared provider unconditionally: `embed::Provider::from_settings(...)?.context("documentation embedding requires the repository [embedding] provider; BM25 search does not")?` (`src/commands/docs.rs:87-93`), so a repository with no configured provider is a hard error rather than a no-op. It opens for write and calls `retrieval::embed_current`. The consequence to know: `[docs.search] vector = true` on a repository that never ran `docs embed` yields BM25-only results with a not-ready vector generation, silently.
...
sequenceDiagram
  ...
  D-->>U: human lines, compact JSON, or pretty diagnostics
```
...
`open_path_read_only` (`src/store.rs:63`) refuses to create a file — a missing database bails with "run `jscout index` first" (`:64-69`) — sets `query_only`, and then rejects in a fixed order: schema version against `SCHEMA_VERSION = "31"` (`src/store.rs:8`), a published `snapshot` row together with `projection_version` matching `structural::PROJECTION_VERSION`, then via `validate_published_contracts` (`:123`) both `extraction_version` against `entity::EXTRACTION_VERSION` and `documentation_chunk_format_version` against `docs::CHUNK_FORMAT_VERSION`. Every rejection names `jscout index` as the repair. The comment at `:56-58` states the reason plainly: a typo or an unindexed checkout must not silently create a database that looks usable.
...
`src/agent.rs:7` embeds `integrations/jscout/SKILL.md` at compile time as `GUIDE`. `agent-guide` prints it to stdout; `agent-guide --install ROOT` canonicalizes the root, creates `.agents/skills/jscout/`, and opens the target with `create_new(true)` (`src/agent.rs:9-30`), so an existing project-local guide is an error, never an overwrite. A repository may have edited its copy, and discarding that silently would be worse than requiring a manual delete before upgrading.

The guide was restructured for G23 from a flat bullet list into two named, numbered loops plus a new leading section. The **Investigation loop** (`SKILL.md:28`) covers known identifiers, conventions, and blast radius: start with `semantic_search` at `exhaustive: true`, treat `limit` as a page size and never as a completeness boundary, traverse pages by echoing `next_cursor` unchanged, and stop only when `truncated` is false *and* the summed page-local `returned` equals `total_chunks`. It also pins a deterministic recovery: on the marker `response_budget_too_small: … minimum_bytes=<N>`, retry the same page with `response_bytes: N` and the input cursor unchanged, because the error itself is not progress. The **Inquiry loop** (`:88`) covers causal and cross-file questions: begin at `semantic_memory`, read one artifact at a time with its exact `view=body` arguments, then set `include_memory: false` and `expand: false` on subsequent localization searches so the same artifact is not repeatedly attached. The new leading section, "Authored repository documentation" (`:14`), states that `documentation_search` shares the repository snapshot but ranks in a separate corpus, and that a documentation hit is authored prose rather than runtime proof.
