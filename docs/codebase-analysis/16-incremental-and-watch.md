...
Publication is two closures inside one `BEGIN IMMEDIATE` (`:440`). The preparation closure extracts, sweeps unseen rows, runs dependency discovery and reads *before* deleting the publication markers, and only then unpublishes them. The publication closure synchronizes dependency instances, indexes dependency files, materializes cached embeddings when occurrences changed, resolves module edges, computes the component identities and publication fold, applies checker retention, then either reuses or rebuilds the structural projection. Documentation vector generations are rematerialized when extraction reset, the documentation digest changed, or a current profile lacks readiness. Any error rolls the whole thing back; WAL readers keep the previous publication throughout.
