...
jscout turns each JS/TS file into flat, source-local rows — symbols, imports, exports, references, member calls, events, entity sites, value flows — using exactly one oxc parse, and then, in a separate pass that runs after every file is stored, resolves those rows against the repository-wide module graph into a disposable `graph_nodes` / `resolved_edges` traversal graph where every edge carries an explicit confidence and provenance. The split is deliberate: extraction knows nothing about any other file and can be cached per file hash, while projection is thrown away and rebuilt whenever anything about the snapshot changes. Layered on top are two things worth reading carefully — TypeScript's type layer is erased from the runtime plane and re-emitted as a parallel *documentary* contract plane that traversal refuses to walk, and a small, aggressively bounded value-flow pass that proves receiver identity for a minority of member calls and, when it succeeds, permanently removes those call sites from the TypeScript checker's work plan.

## The two halves and their version constants

Extraction runs inside `parse::with_parsed` (`src/parse.rs:26`), which parses with oxc — JSX forced on for every JavaScript source type (`src/parse.rs:16`) — and builds `Semantic` with `with_build_nodes(true)` (`src/parse.rs:45`) because reference classification walks node ancestors. `graph::extract` (`src/graph.rs:71`) is the sole entry point, called once per file from `indexer::extract_file` (`src/indexer.rs:841`). It reads oxc's `module_record` directly for imports and exports, and calls three visitors: `heur::extract` (`src/heur.rs:285`), `entity::extract` (`src/entity.rs:1073`), and `value_flow::extract` (`src/value_flow.rs:82`). The result is one owned `FileGraph` (`src/graph.rs:50-69`) which `indexer::insert_file` (`src/indexer.rs:1070`) flattens across seventeen writes: `files` → `chunks` + `chunks_fts` → `symbols` → `imports` → `exports` → `contract_imports` → `contract_exports` → `events` → `member_calls` → `receiver_value_flows` → `function_return_flows` → `value_binding_flows` → `class_value_flows` → `instance_method_value_flows` → `class_member_value_flow_blockers` → `entity_sites` → `refs`. None of `graph.rs`, `entity.rs`, `heur.rs`, or `value_flow.rs` mentions `rusqlite` or another file's contents; source-locality is enforced by what those modules can even reference.
...
| --- | --- | --- | --- |
| `entity::EXTRACTION_VERSION` | `"7"` | `src/entity.rs:14` | `UPDATE files SET hash='' WHERE corpus='code'`, plus `resolved_edges`, `graph_nodes`, and the `snapshot`/`projection_version` meta keys (`src/indexer.rs:790-816`) — forces every code file to be re-parsed |
| `structural::PROJECTION_VERSION` | `"12"` | `src/structural.rs:13` | Only the disposable graph, via the snapshot digest |

The name `EXTRACTION_VERSION` is misleading about its scope: it lives in `entity.rs` but governs *every* per-file extraction output, value flow included, and is hashed into the structural snapshot under the domain separator `code-extraction-contract` (`src/structural.rs:436-437`). Its invalidation is corpus-scoped — documentation rows keep their hashes.
...
`rebuild_projection_with_timing` opens `SAVEPOINT jscout_structural_projection` (`src/structural.rs:515`) rather than `BEGIN IMMEDIATE`, with the comment explaining that projection replacement must be atomic both standalone and as one stage of the indexer's outer publication transaction. Inside it: load files, module graph, and symbols (517-528); delete `resolved_edges`, `graph_nodes`, and `entities` (530-532, with occurrences and entity edges cascading); insert one node per file and one per symbol (539-567); run seven stages; then upsert `meta.snapshot` and `meta.projection_version` (640-649). Success releases the savepoint, error rolls back to it and re-raises, leaving the previous snapshot intact (652-660). The tradeoff of the savepoint form is that the three loads now happen *inside* it, so the write lock is held across them.
...
flowchart TD
  ...
  S7 --> META["upsert meta.snapshot + meta.projection_version"]
  META --> REL["RELEASE"]
...
`compute_snapshot_with_resolution` (`src/structural.rs:429`) hashes, under the domain `jscout-structural-snapshot-v4`: `PROJECTION_VERSION`, `entity::EXTRACTION_VERSION`, the persisted `meta.extraction_version`, `docs::CHUNK_FORMAT_VERSION`, the persisted `meta.documentation_chunk_format_version`, then every `files` row (path, hash, role, origin, corpus, format, package fields, ordered by path) and the resolution hash. Hashing both the in-binary contract and the persisted marker that describes the stored rows is what makes an interrupted or pre-upgrade run fail closed rather than present an apparently current digest (comments at 446-447 and 460-462).

Two consequences follow, one of them uncomfortable. First, `load_files` reads the `code_files` view (`src/structural.rs:688`) — `SELECT file.* FROM files WHERE file.corpus='code'` (`src/store.rs:429-432`) — and `ModuleGraph` does the same (`src/query.rs:97`), so documentation rows never become graph nodes or participate in export resolution. Three projection queries nevertheless join the raw `files` table (`project_module_edges` at 767, `project_checker_enrichments` at 2260 and 2273, `checker_occurrence_coverage` at 2176); doc rows are excluded there by the downstream `files.get(&id)` map lookup, not by the SQL. Second, and more consequential: the snapshot query has **no corpus filter**. Editing a single `.md` file changes `files.hash` for that row, changes the snapshot digest, and therefore forces a full projection rebuild — and because `project_checker_enrichments` requires `batch.source_snapshot=?1`, every checker-provenance edge silently drops out until a new checker batch is built. The two corpora are separated at the row level and coupled at the identity level.
