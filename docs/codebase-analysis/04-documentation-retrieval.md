...
So the separate database, the `[docs.database]` key, and the docs-scoped snapshot sequence all died. What shipped instead is one walker, one snapshot, one index pass, one watcher, and one entity plane — with `docs_fts`, `vec_doc_embeddings_{N}`, and a `documentation_search` surface layered on top. The single-traversal property described in `03-documentation-corpus.md` is the same decision seen from the other end. It first shipped as an ownership inversion — `walk::repository_inventory` delegating to `docs::corpus::scan_repository` — and `b92485c` plus `03d5b50` (PR #108) reversed the dependency direction without weakening the property. The traversal engine now lives in `src/walk/inventory.rs` (263 lines); `walk::repository_inventory` is generic over a `RepositoryInventoryConsumer` (`src/walk.rs:58`, entry point at `:187`), and `docs::corpus` implements that trait for `DocumentationCollector` (`src/docs/corpus.rs:422`), reaching it through its own `corpus::repository_inventory` wrapper (`src/docs/corpus.rs:156`). One deterministic traversal still produces both file lists; the documentation module rides it rather than owning it, and `src/walk.rs` carries no module-level `use crate::docs` at all.
...
- **Markdown edits do not trigger a watch generation.** `EventClassifier` marks source dirty only for `walk::is_indexable` paths, whose `EXTENSIONS` list is JS/TS only (`src/walk.rs:10`), so documentation is reindexed only when a code change happens to trigger a generation. Documentation-aware watch classification is G24 phase 4 and is not built; the classifier ladder is in `16-incremental-and-watch.md`.
