...
flowchart TB
  ...
  GEN["doc_vector_generations per snapshot"]
...
- **Readiness is a generation row, not a meta marker.** `doc_vector_generations(snapshot, profile_id, dimensions, chunk_format_version)`, all four columns in the primary key. Docs vectors must be exactly current with the published structural snapshot, so `rematerialize_cached_generations` runs inside the indexer's publication transaction when the snapshot changed (`src/indexer.rs:718-720`) — provider-free, with an incomplete cache treated as a normal NotReady state. That hook first calls `clear_all_materialized_generations` (`src/docs/retrieval.rs:867-885`), wiping every docs vec0 table, all entries, and all generation rows before rebuilding.
...
Docs search reports three vector outcomes rather than two — `NotReady` (no profile, or no generation for this snapshot) is distinct from `Degraded` (an error), and `vector_required` converts either into a hard failure (`src/docs/retrieval.rs:412-453`). Its `reranker_document` (`src/docs/retrieval.rs:214`) composes `path / title / description / tags / breadcrumb` and the rendered body, then goes to the same `/rerank` endpoint.
...
- `delete_vector_rows_for_file` (`src/embed.rs:1826`) deletes **all** `doc_vector_generations` rows for the affected profile, not just the current snapshot's. One deleted `.md` file invalidates that profile's documentation vector readiness entirely.
