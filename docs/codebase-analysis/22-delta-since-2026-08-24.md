...
## Addendum: the ownership inversion was reversed

Everything above describes the range `4de5622..823b836` and stays accurate for it. One finding did not survive the next commit, and a reader who stops here would carry a wrong model of `main`.

PR #108 — `b92485c` ("refactor: centralize shared repository inventory") and `03d5b50` ("refactor: make repository walk plane-agnostic"), both on 08-25, merged as `ebacdf1` — moved the traversal engine out of `docs/corpus.rs` into a new `src/walk/inventory.rs` and made `walk::repository_inventory` generic over a `RepositoryInventoryConsumer` trait. The shared-traversal property this document reports is intact; the arrow between the two modules is not.

| | `823b836` (this range) | `ebacdf1` |
|---|---|---|
| Traversal engine | `src/docs/corpus.rs`, scanner method | `src/walk/inventory.rs`, 263 lines |
| Dependency direction | `walk` → `docs::corpus` | `docs::corpus` → `walk` (`src/docs/corpus.rs:156`, `:422`) |
| `walk`'s knowledge of docs | `CapturedDocument`, `CorpusOptions`, `Decision` in its result type | none at module level; the sole `docs` reference is inside `#[cfg(test)]` at `src/walk.rs:244` |
| Result type | `RepositoryInventory { files, rejections, documents, documentation_decisions }` | `RepositoryInventory<T> { files, rejections, consumer: T }` (`src/walk.rs:49`) |
| Production entry point | `walk::repository_inventory(root, &CorpusOptions)` | `corpus::repository_inventory` (`src/docs/corpus.rs:156`) calling `walk::repository_inventory` (`src/walk.rs:187`) |
| File sizes | `walk.rs` 501, `corpus.rs` 2,708 | `walk.rs` 506, `walk/inventory.rs` 263, `corpus.rs` 2,548 |

So the claim in "The ownership inversion" above — that `walk::repository_inventory` no longer walks anything and that the documentation module owns the single traversal producing both planes — is true of `823b836` and false of `main`. At `main`, `walk` owns the traversal, the ignore matcher, the two descent bits, and the code-file list; `docs::corpus` owns a consumer that rides the traversal and keeps its own globs, hidden-path policy, capture, parsing, and decision log. The trait is `pub(crate)` with one implementor, so the abstraction buys the dependency inversion and nothing else. Two figures also moved with it: `.rs` files under `src/` 87 → 88, and Rust LOC under `src/` 91,174 → 91,282, the new `src/walk/inventory.rs` netting against what `corpus.rs` lost. Schema, tests, tables, and MCP tools are unchanged — `SCHEMA_VERSION` is still `"31"`, still 570 `#[test]` attributes, still 47 tables.

`03-documentation-corpus.md` and `04-documentation-retrieval.md` in the current set describe the post-#108 arrangement directly; this addendum exists only so the delta's own narrative does not strand a reader on the superseded one.

## Addendum: PR #117 search response defaults (2026-09-05)

PR #117 raises ranked source excerpts to eight lines / 1,024 UTF-8 bytes and
the default complete code-search response ceiling from 24,000 to 30,000 JSON
bytes. Explicit repository and per-call limits still take precedence; existing
config files are not rewritten. Non-search tools retain 24,000 bytes through
one `compact::DEFAULT_TOOL_RESPONSE_BYTES` constant shared by MCP schemas,
runtime fallbacks, and CLI/module defaults. Documentation search and expansion
budgets remain independent and unchanged. The current `10-code-retrieval.md`,
`14-mcp-surface.md`, and `15-configuration.md` reflect these defaults; earlier
figures in this delta remain historical. Snippet shedding, window scoring,
and relation-preview behavior are unchanged by this cleanup.

## Addendum: scoped search and broad-OR confirmation (2026-09-07)

The search-waste follow-up changes three query-layer contracts without an
index rebuild. Earlier trace and implementation descriptions in this analysis
remain historical; [G22/G23](../../PLAN.md#implemented-g22--exhaustive-lexical-search-contract)
and the [command reference](../commands.md#search-scope-and-exhaustive-results)
define the current behavior.

- Code and documentation search now accept exact `path` and directory
  `path_prefix` scope before lexical, vector, and exact-identifier candidate
  limits. Empty/absent query is allowed only with path scope and returns
  bounded deterministic indexed chunks without inference, freshness, memory,
  or expansion. Textual ranked search meaning is unchanged.
- Exhaustive text search defaults to `match_mode: "all"`: AND within a
  single content chunk. Explicit `"any"` preserves the old OR meaning. The
  effective operator and normalized path scope join the cursor fingerprint;
  cursors from the old query contract are not accepted as new traversals.
- An initial unconfirmed multi-token OR query with at least 200 scoped chunks
  returns the count, `confirmation_required: true`, `returned: 0`,
  `truncated: true`, and no cursor. `allow_broad: true` starts the ordinary
  complete pagination; continuation needs only its valid cursor, not another
  confirmation flag. The `broad_or_query` warning is now actionable before
  hit delivery, rather than a warning beside an already materialized page.

Documentation text search has no exhaustive operator or broad-query guard.
An explicit expansion on a ranked code text query remains related context,
separately labeled and allowed outside the primary path scope.
