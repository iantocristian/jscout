...
`src/walk/inventory.rs` is the traversal engine, and `RepositoryInventoryConsumer` is the seam a second plane rides through. Production documentation indexing builds a `DocumentationCollector` and hands it to generic `walk::repository_inventory`. Markdown membership, capture, and parse happen inside the same deterministic traversal that selects code paths, so there is no second filesystem scan that could disagree with the code inventory inside one publication. This does **not** mean the two corpora share an invalidation identity: G27 computes independent code and documentation digests from their published plane state. `publication_snapshot` is the fold of those components plus provenance and is not itself a gate.
...
The old `walk::source_inventory` survives for file-list diagnostics and dependency discovery, while watch uses standalone policy matchers rather than either inventory. The two inventory implementations must keep code membership aligned, which `shared_inventory_preserves_the_source_walker_contract` tests. Documentation watch classification is separate: `EventClassifier` consults `DocumentationPathPolicy` before code format classification, so an admitted `.md` or `.mdx` event schedules a documentation generation.
